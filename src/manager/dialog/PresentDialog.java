		...
		btnNewButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				try{
					...
					dialog.setMessage("本当にギフトを送りますか？");
					...
					if(target != null){
	                    ...
	                    if (temp.isStackable()) {
	    					...
	    					if (target .getInventory().checkAddItem(item, count) == L1Inventory.OK) {
	    						...
	    						//sendPackets(new S_SystemMessage(item.getViewName()+ "を贈りました。"));
	    					}
	    				} else {
	    					...
	    					if (createCount > 0) {
	    						//sendPackets(new S_SystemMessage(item.getViewName()+ "を" + createCount + "個贈りました。"));
	    					}
	    				}
	                    MessageBox dialog2 = new MessageBox(shell, SWT.CANCEL|SWT.ICON_INFORMATION);
	    				dialog2.setText("Present");
	    				dialog2.setMessage("正常に贈られました。");
	    				dialog2.open();

					} else {
						MessageBox dialog2 = new MessageBox(shell, SWT.CANCEL|SWT.ICON_INFORMATION);
						dialog2.setText("Present");
						dialog2.setMessage(text.getText() + "はワールドに存在しません。");
						dialog2.open();
					}
				} catch(Exception e1){
					MessageBox dialog2 = new MessageBox(shell, SWT.CANCEL|SWT.ICON_INFORMATION);
					dialog2.setText("Present");
					dialog2.setMessage("値が無効です。");
					dialog2.open();
				}
			}
		});
