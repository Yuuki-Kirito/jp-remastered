...
public class MJLFCObject extends MJInstanceObject {
	...
	protected void waitCount() throws Exception {
		try {
			String sec = new StringBuilder().append(_lType.getReadySecond()).append("秒後、試合が始まります。").toString();
			...
			for (int i = _lType.getReadySecond(); i > 0; i--) {
				if (i <= 10) {
					...
					msg.append(i).append("秒後、試合が始まります。");
	...
	protected boolean checkSecond() {
		if (_time > 0) {
			...
			if (_time <= 10 && _time != 0) {
				String sec = String.format("%d秒後、試合は終了します。", _time);
