...
public abstract class Warehouse extends L1Object {
	...
	/*
	 * private void reload (){ for (L1ItemInstance item : _items) {
	 * L1World.getInstance().removeObject(item); } _items.clear(); }
	 */
	...
	public synchronized L1ItemInstance getItem(int objectId, int count) {
		L1ItemInstance item = null;
		for (Object itemObject : _items) {
			item = (L1ItemInstance) itemObject;
			/*
			 * if(item !=null){ if(!checkitem(item.getItemId(),count)){
			 * reload(); loadItems(); return null; } }
			 */
