...
public class PrivateWarehouse extends Warehouse {
	...
	public synchronized boolean checkitem(int itemid, int count) {
		...
		try {
			...
			pstm = con.prepareStatement("SELECT count FROM character_warehouse WHERE account_name = ? AND item_id = ?");
	...
	public synchronized void insertItem(L1ItemInstance item) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm2 = con.prepareStatement("SELECT * FROM character_warehouse WHERE id = ?");
			pstm2.setInt(1, item.getId());
			rs = pstm2.executeQuery();
			if (!rs.next()) {
				pstm = con.prepareStatement(
						"INSERT INTO character_warehouse SET id = ?, account_name = ?, item_id = ?, item_name = ?, count = ?, is_equipped=0, enchantlvl = ?, is_id = ?, durability = ?, charge_count = ?, remaining_time = ?, last_used = ?, attr_enchantlvl = ?, step_enchantlvl = ? , bless = ?, second_id=?, round_id=?, ticket_id=?, end_time=?, CreaterName=?, demon_bongin=?");
