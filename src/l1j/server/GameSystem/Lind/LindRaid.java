...
public class LindRaid {
	...
	public synchronized boolean start(L1PcInstance pc) {
		...
		ArrayList<L1Object> list = L1World.getInstance().getVisibleObjects(pc,
				0);
		if (list.size() > 0) {
			pc.sendPackets(new S_SystemMessage("この場所にリンドポータルを召喚できません。"),
					true);
			return false;
		}
		L1World.getInstance()
				.broadcastServerMessage(
						"スチールギルドドワーフ：う…ドラゴンの泣き声がここまで聞こえる。 いつも誰かがドラゴンポータルを開いたことが確実です！ 用意されたドラゴンスレイヤーに栄光と祝福を！");
