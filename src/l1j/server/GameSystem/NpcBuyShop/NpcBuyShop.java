...
public class NpcBuyShop {
	...
	public void delete_shop_item(int npcid, int objid) {// 아이템아이디별 판매 구매 구분후 삭제
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
	...
	public void SaveShop(L1NpcInstance npc, L1ShopItem item, int price,
		...
		try {
			...
			System.out.println("重複 objid -> " + npc.getName() + " >>>> [" + item.getItem().getName() + "] ID : " + item.getId());
			e.printStackTrace();
		} catch (Exception e) {
			System.out.println("重複 objid -> " + npc.getName() + " >>>> [" + item.getItem().getName() + "] ID : " + item.getId());
			e.printStackTrace();
	...
	public synchronized void insert(int itemid, int enchant, int attr,
		...
		while (count-- > 1) {
			...
			if (ns.getDefaultName() != null && ns.getShopName() != null
				...
				if (s == null || s.getSellingItems().size() < 5) {// 無条件に1つずつ
																	// 売れるように変更。
					ck = false;
					break;
				}
		...
		if (ck || ns == null) {
			S_SystemMessage sm = new S_SystemMessage("> 購入による販売店がないか不足しています。");
			...
			sm = null;
			System.out.println("> 購入による販売店がないか不足しています。");
			return;
		}
		...
		if (item == null) {
			System.out.println(":: 買取店 ローディング中 価格のないアイテム >> " + itemid);
			return;
		}
		if (time != null)
	 ...
	 * 32955 //1方向 32817 32826 32955 32970 32803 32813 32955 32970 32788 32798
