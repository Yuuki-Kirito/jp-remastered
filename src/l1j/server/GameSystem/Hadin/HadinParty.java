...
public class HadinParty {
	public HadinParty(L1Party p, short _mapid) {
		...
		for (L1NpcInstance npc : BasicNpcList) {
			if (npc != null) {
				if (npc.getName().equalsIgnoreCase("ハーディン")) {
					Npc_Hadin = npc;
				} else if (npc.getName().equalsIgnoreCase("ハーディンフロアエフェクト")) {
					Hadin_Effect = npc;
				} else if (npc.getName().equalsIgnoreCase("ケレニス")) {
					Kere = npc;
				}
		...
		for (L1NpcInstance npc : Leader_BasicNpcList) {
			if (npc.getSpawnLocation().equalsIgnoreCase("オーク"))
				Npc_Orc = npc;
			else if (npc.getSpawnLocation().equalsIgnoreCase("オークファイター"))
				Npc_OrcWarrior = npc;
			else if (npc.getSpawnLocation().equalsIgnoreCase("オークアーチャー"))
				Npc_OrcAcher = npc;
