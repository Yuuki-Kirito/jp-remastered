		...
		try {
			...
			while (rs.next()) {
				...
				while (s.hasMoreElements()) {
					...
					while (values.hasMoreElements()) {// 空白の削除
					...
					if (number == 0) { // 天気月火水木~~
						...
						while (mdata.hasMoreElements()) {
							...
							StringTokenizer Hours = new StringTokenizer(Times, "市");
						...
						// System.out.println("スポーン座標x = " + temp.SpawnLoc[0]);
						// System.out.println("スポーン座標y = " + temp.SpawnLoc[1]);
						// System.out.println("スポーン座標m = " + temp.SpawnLoc[2]);
					}
