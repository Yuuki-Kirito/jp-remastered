...
public class GambleInstance extends L1NpcInstance {
			...
			if (!npc.Npc_trade && !play) {
				if (!npc.isParalyzed()) {
					if (player.getTradeReady())
						return;
					player.setTradeID(getId()); // ����� ������Ʈ ID�� ������ �д�
	...
	private static String chat9 = "ああ！ おめでとうございます...配当金を支払いました...";
	private static String chat11 = "残念ですね…次の機会に挑戦してください～！";
		...
		public void run() {
			try {
				if (npc == null || pc == null || item_count == 0)
					return;
				String chat = pc.getName() + "�� " + item_count
						+ "アデンバッティングしました〜1匹合わせるたびに5倍です〜！";
				...
				while (count-- > 0) {
					...
					if (pc.Gamble_Text == null || pc.Gamble_Text.equals("")) {
						Broadcaster.broadcastPacket(npc, new S_NpcChatPacket(
								npc, "賭けるべき名前 " + (count * 4) + "秒以内に教えてください〜",
								0), true);
					} else
						break;
				}
				if (pc.Gamble_Text == null || pc.Gamble_Text.equals("")) {
					...
					Broadcaster.broadcastPacket(npc, new S_NpcChatPacket(npc,
							"60秒以内にあなたの名前を入力しないで、Adenはただ持っています〜", 0), true);
					return;
				}
				Broadcaster.broadcastPacket(npc, new S_NpcChatPacket(npc,
						pc.Gamble_Text + "に賭けます〜", 0), true);
				Thread.sleep(2000);
				if (ck())
					return;
				Broadcaster.broadcastPacket(npc, new S_NpcChatPacket(npc,
