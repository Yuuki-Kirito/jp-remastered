...
public class S_IdentifyDesc extends ServerBasePacket {
	...
	private void buildPacket(L1ItemInstance item) {
		...
		if (item.getItem().getType2() == 1) { // weapon
			...
			/*
			 * if (item.getItem().getType() == 1) { // wand writeH(137); //
			 * \f1%0：사용 가능 회수%1［무게%2］ writeC(3); writeS(name.toString());
			 * writeS(String.valueOf(item.getChargeCount())); } else
			 */if (item.getItem().getItemId() != 7338
