...
public class ElementalStoneGenerator implements Runnable {
	...
	// Logger.getLogger(ElementalStoneGenerator.class.getName());
	...
	private boolean canPut(L1Location loc) {
		...
		if (!_dummy.getMap().isInMap(_dummy.getX(), _dummy.getY())
				|| !_dummy.getMap().isPassable(_dummy.getX(), _dummy.getY()))
			return false;
		return true;
	}
