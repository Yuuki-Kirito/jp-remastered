...
public class L1ParalysisPoison extends L1Poison {
	...
	private class ParalysisPoisonTimer extends Thread {
		private ParalysisPoisonTimer() {
			super(
					"l1j.server.server.model.poison.L1ParalysisPoison.ParalysisPoisonTimer");
		}
		...
		public void run() {
			try {
				_target.getSkillEffectTimerSet().setSkillEffect(
						L1SkillId.STATUS_POISON_PARALYZING, 0);

				try {
					Thread.sleep(_delay);
				} catch (InterruptedException e) {
					_target.getSkillEffectTimerSet().killSkillEffectTimer(
							L1SkillId.STATUS_POISON_PARALYZING);
					return;
				}
				...
				if (_target instanceof L1PcInstance) {
					...
					if (player.isDead() == false) {
						player.sendPackets(new S_Paralysis(1, true), true);
		...
		public void run() {
			try {
				_target.getSkillEffectTimerSet().killSkillEffectTimer(
						L1SkillId.STATUS_POISON_PARALYZING);
				_target.getSkillEffectTimerSet().setSkillEffect(
						L1SkillId.STATUS_POISON_PARALYZED, 0);
				try {
					if (_target instanceof L1PcInstance)
						((L1PcInstance) _target).sendPackets(new S_PacketBox(
								S_PacketBox.커스종류, 2, _time / 1000), true);
					Thread.sleep(_time);
				} catch (InterruptedException e) {
				}
				...
				if (_target instanceof L1PcInstance) {
					L1PcInstance player = (L1PcInstance) _target;
					if (!player.isDead()) {
						player.sendPackets(new S_Paralysis(1, false), true);
						cure();
					}
	...
	private void doInfection() {
		...
		if (_target instanceof L1PcInstance) {
			((L1PcInstance) _target).sendPackets(new S_PacketBox(
					S_PacketBox.커스종류, 2, (_delay) / 1000), true);
