...
public class L1TaxCalculator {
	...
	public L1TaxCalculator(int merchantNpcId) {
		/** 특정 NPC 정해진 성에서 세율 받아오기 **/
		if (merchantNpcId == 70030 || merchantNpcId == 4500166) {
			_taxRatesCastle = 4;
		} else if (merchantNpcId == 4500171 || merchantNpcId == 4200102) {
			_taxRatesCastle = 1;
		} else {
	...
	public int calcTotalTaxPrice(int price) {
		...
		/*
		 * int taxTown = (price * _taxRatesTown) / 100; int taxWar = (price *
		 * WAR_TAX_RATES) / 100; return taxCastle + taxTown + taxWar;
		 */
		return taxCastle;
	}
