		...
		if (cha instanceof L1PcInstance) {
			...
			case 60151:
				...
				if (pc.get_food() < 225) { // 100%
					...
					pc.set_food(pc.get_food() + (useItem.getItem().getFoodVolume() / 10) * chargeCount);// 아이템마다의 영양으로 체워지게
					if (pc.get_food() >= 225) {
						pc.set_food(225);
						pc.setSurvivalCry(System.currentTimeMillis());
					}
				}
				pc.sendPackets(new S_PacketBox(S_PacketBox.FOOD, pc.get_food()), true);
				...
				pc.sendPackets(new S_ServerMessage(76, useItem.getItem().getNameId()), true);
				break;
			default:
				break;
			}
			if ((itemId >= 41277 && itemId <= 41292) || (itemId >= 49049 && itemId <= 49064)
			...
			} else if ((itemId >= 60072 && itemId <= 60075) || (itemId >= 60726 && itemId <= 60729) || (itemId >= 600350 && itemId <= 600353) || itemId == 60259
					|| itemId == 60260) {
