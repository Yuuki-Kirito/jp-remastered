...
public class FurnitureItem extends L1ItemInstance {
	...
	private void useFurnitureItem(L1PcInstance pc, int itemId, int itemObjectId) {
		if (!L1HouseLocation.isInHouse(pc.getX(), pc.getY(), pc.getMapId())) {
			pc.sendPackets(new S_ServerMessage(563), true); // \f1 여기에서는 사용할 수
															// 없습니다.
			return;
		}
		int x = pc.getX() + HEADING_TABLE_X[pc.getMoveState().getHeading()];
		int y = pc.getY() + HEADING_TABLE_Y[pc.getMoveState().getHeading()];
		if (!L1HouseLocation.isInHouse(x, y, pc.getMapId())) {
			pc.sendPackets(new S_ServerMessage(563), true); // \f1 여기에서는 사용할 수
															// 없습니다.
			return;
		}
