...
public class L1WarSpawn {
	...
	public void SpawnFlag(int castleId) {
		...
		for (x = locx1, y = locy1; x <= locx2; x += 8) {
			SpawnWarObject(l1npc, x, y, mapid, gfx);
		}
		for (x = locx2, y = locy1; y <= locy2; y += 8) {
			SpawnWarObject(l1npc, x, y, mapid, gfx + 2);
		}
	...
	private void SpawnWarObject(L1Npc l1npc, int locx, int locy, short mapid) {
		SpawnWarObject(l1npc, locx, locy, mapid, 0);
	}
	...
	private void SpawnWarObject(L1Npc l1npc, int locx, int locy, short mapid,
		...
		try {
			if (l1npc != null) {
				...
				npc.setMap(mapid);
				if (gfx != 0 && gfx != 2) {
					npc.getGfxId().setTempCharGfx(gfx);
				}
				// System.out.println(locx+" "+locy);
				// int tile = L1Map.newNull().getTile(locx, locy);
				// System.out.println(npc.getMap().getTile(locx, locy));
				if (npc.getMap().getTile(locx, locy) == 0) {
					return;
				}
