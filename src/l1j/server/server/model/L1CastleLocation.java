...
public class L1CastleLocation {
	...
	// 켄트성
	/*
	 * private static final int KENT_TOWER_X = 33170; private static final int
	 * KENT_TOWER_Y = 32774; private static final short KENT_TOWER_MAP = 4;
	 */
	...
	// 윈다우드
	/*
	 * private static final int WW_TOWER_X = 32675; private static final int
	 * WW_TOWER_Y = 33408; private static final short WW_TOWER_MAP = 4;
	 */
	...
	public static int getCastleIdByInnerArea(L1Location loc) {
		for (Map.Entry<Integer, Integer> entry : _innerTowerMaps.entrySet()) {
			if (entry.getValue() == loc.getMapId()) {
				return entry.getKey();
			}
	...
	public static int getCastleIdByInMap(int catleid) {
		for (Map.Entry<Integer, Integer> entry : _innerTowerMaps.entrySet()) {
			if (entry.getKey() == catleid) {
	...
	public static int[] getCastleLoc(int castle_id) { // / castle_id로부터 키우치의 좌표를
														// 돌려준다
		int[] loc = new int[3];
		switch (castle_id) {
		case KENT_CASTLE_ID:
	...
	public static void setCastleTaxRate() {
		for (L1Castle castle : CastleTable.getInstance().getCastleTableList()) {
			...
			int plusmoney = castle.get_publicPlusMoney();
			castle.setPublicMoney(sum);
			castle.setShowMoney(prm);
			castle.set_publicPlusMoney(plusmoney + prm);
			castle.set_publicPlus_D_Money(0);
	...
	private static class L1CastleTaxRateListener extends GameTimeAdapter {
		...
		public void onHourChanged(BaseTime time) {
			...
			for (L1Castle castle : CastleTable.getInstance()
					.getCastleTableList()) {
				if (castle.getId() == 1) {
					// 단위 만, 하루2000~3000 (게임시간은 6배 빠름 즉.게임시간 6일이 현실 1일
					// 게임6시간:현실1시간 게임1시간:현실10분)
					// 하루 4166666~6000000 시간 173611~250000 분 2893 ~ 4166
					castle.setPublicReadyMoney(castle.getPublicReadyMoney()
							+ (rnd.nextInt(69445) + 138888));
				}
			}
			rnd = null;
		}
