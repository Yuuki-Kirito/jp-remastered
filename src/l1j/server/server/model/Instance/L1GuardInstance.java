...
public class L1GuardInstance extends L1NpcInstance {
	...
	/*
	 * @Override public void searchTarget() { L1PcInstance targetPlayer = null;
	 * 
	 * for (L1PcInstance pc : L1World.getInstance().getVisiblePlayer(this)) {
	 * 
	 * if (pc.getCurrentHp() <= 0 || pc.isDead() || pc.isGm() || pc.isGhost()) {
	 * continue; } if (!pc.isInvisble() || getNpcTemplate().is_agrocoi()) { if
	 * (pc.isWanted()) { targetPlayer = pc; break; } } }
	 * 
	 * if (targetPlayer != null) { _hateList.add(targetPlayer, 0); _target =
	 * targetPlayer; } }
	 */
		...
		if (_target == null
			...
			if (_target != null) {
				...
				Broadcaster.broadcastPacket(this,
						new S_SkillSound(getId(), 169), true);
