...
public class L1MerchantInstance extends L1NpcInstance {
    ...
    public void onTalkAction(L1PcInstance player) {
        ...
        if (talking != null) {
            ...
            if (npcid == 100219) {// 수련장 관리인
                ...
                if (player.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.초보자도우미클릭ディレイ)) {
                    player.sendPackets(new S_SystemMessage(player.getSkillEffectTimerSet().getSkillEffectTimeSec(L1SkillId.초보자도우미클릭ディレイ) + "초 뒤에 사용 가능합니다."), true);
                    return;
                }

                player.getSkillEffectTimerSet().setSkillEffect(L1SkillId.초보자도우미클릭ディレイ, 5000);
    ...
    public boolean 공성병기ディレイ = false;

    public void 공성병기투척ディレイ() {
        if (공성병기ディレイ) {
			return;
		}
