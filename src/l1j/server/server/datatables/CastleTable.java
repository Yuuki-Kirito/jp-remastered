...
public class CastleTable {
	...
	private void load() {
		...
		try {
			...
			while (rs.next()) {
				...
				castle.set_publicPlusMoney(rs.getInt(10));
				castle.set_publicPlus_D_Money(rs.getInt(11));
	...
	public void updateCastle(L1Castle castle) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("UPDATE castle SET name=?, war_time=?, tax_rate=?, public_money=?, public_ready_money=?, show_money=?, war_basetime=?, security=?, plus_money=?, pluse_d_money=? WHERE castle_id=?");
			pstm.setString(1, castle.getName());
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
			String fm = sdf.format(castle.getWarTime().getTime());
			// String fm = DateFormat.getDateTimeInstance().format( //## A1 オリジナル
			...
			pstm.setInt(9, castle.get_publicPlusMoney());
			pstm.setInt(10, castle.get_publicPlus_D_Money());

			pstm.setInt(11, castle.getId());
			pstm.executeUpdate();
