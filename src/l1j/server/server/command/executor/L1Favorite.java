	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
			...
			if (arg.startsWith("셋팅")) {
				...
				st.nextToken();
				if (!st.hasMoreTokens()) {
					pc.sendPackets(new S_SystemMessage("커멘드가 없습니다. "), true);
					return;
				}
				...
				pc.sendPackets(new S_SystemMessage("등록하고 있는 커멘드가 없습니다. "), true);
			} else {
				StringBuilder cmd = new StringBuilder();
				StringTokenizer st = new StringTokenizer(arg);
				...
				pc.sendPackets(new S_SystemMessage(cmd + " 를 실행합니다. "), true);
