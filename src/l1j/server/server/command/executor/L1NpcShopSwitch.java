...
public class L1NpcShopSwitch implements L1CommandExecutor {
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
			 ...
			 * !pc.getName().equalsIgnoreCase("미소피아") ) { pc.sendPackets(new
			 * S_SystemMessage("운영자 이름이 아니네요.")); return; }
			 */
			if (power)
				pc.sendPackets(new S_SystemMessage("이미 실행중입니다."), true);
			else
				NpcShopSystem.getInstance().npcShopStart();
		} catch (Exception e) {
			pc.sendPackets(new S_SystemMessage("영자상점 커멘드 에러"), true);
		}
	}
}
