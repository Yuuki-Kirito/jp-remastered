...
public class L1Present implements L1CommandExecutor {
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
			...
			if (temp != null) {
				/*
				 * if (!(itemid== 438003)){ pc.sendPackets(new
				 * S_SystemMessage("홍보아이템만 선물 가능")); return; }
				 */
				if (temp.isStackable()) {
				...
				pc.sendPackets(new S_SystemMessage(temp.getName() + "를 "
						+ count + " 개 선물 했습니다. ", true), true);
			} else {
			...
			pc.sendPackets(
					new S_SystemMessage(
							".[선물] [캐릭명] [아이템ID] [갯수] [인챈트수] 로 입력해 주세요. (어카운트명=*으로 모두)"),
					true);
		}
	}
}